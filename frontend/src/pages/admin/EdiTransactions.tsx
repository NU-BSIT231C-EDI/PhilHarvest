...
interface EdiDoc {
  ...
  stControl: string;
  raw?: string;
  /** Backend numeric ID — used for retry */
  backendId: number;
  /** Raw backend status — used to decide if retry is available */
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by Doc ID, company, or type..."
              className="pl-9"
              value={search}
          ...
          <Button
            ...
            onClick={loadDocs}
            disabled={loading}
            data-testid="button-refresh-edi"
          >
              ...
              <tbody className="divide-y divide-border">
                {loading ? (
                  ...
                  <tr key={doc.id} className="hover:bg-muted/30 transition-colors" data-testid={`row-txn-${doc.id}`}>
                    ...
                    <td className="px-4 py-3 text-xs text-foreground max-w-[180px] truncate">{doc.company}</td>
                    <td className="px-4 py-3">
                      <Badge variant={doc.direction === "inbound" ? "secondary" : "outline"} className="text-xs">
                        {doc.direction === "inbound" ? "Inbound" : "Outbound"}
                      </Badge>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center gap-1 justify-end">
                        <Button
                          ...
                          data-testid={`button-view-txn-${doc.id}`}
                        >
                          View
                        </Button>
                        <Button
                          ...
                          title="Download"
                          disabled={!doc.raw}
                          onClick={() => downloadRaw(doc)}
                          data-testid={`button-dl-${doc.id}`}
                        >
            ...
            {selected && (
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><p className="text-xs text-muted-foreground">GS Control #</p><p className="font-mono font-medium">{selected.gsControl}</p></div>
                  <div><p className="text-xs text-muted-foreground">ST Control #</p><p className="font-mono font-medium">{selected.stControl}</p></div>
                </div>
                {selected.raw && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Raw EDI Payload</p>
                    <pre className="bg-muted text-xs p-3 rounded-lg font-mono break-all whitespace-pre-wrap text-foreground overflow-auto max-h-48">
                      {selected.raw}
                    </pre>
                  </div>
                )}
                <div className="flex gap-2 pt-1">
                  <Button size="sm" className="gap-1.5" data-testid="button-validate-doc">
                    <Filter className="w-3.5 h-3.5" />Validate
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    ...
                    onClick={() => downloadRaw(selected)}
                    data-testid="button-download-doc"
                  >
                    <Download className="w-3.5 h-3.5" />Download
                  </Button>
                  {/* Retry — only shown for FAILED outbound transactions */}
                  {selected.backendStatus === 'FAILED' && selected.direction === 'outbound' && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="gap-1.5 text-amber-700 border-amber-300 hover:bg-amber-50"
                      disabled={retrying === selected.backendId}
                      onClick={() => handleRetry(selected)}
                      data-testid="button-retry-doc"
                    >
